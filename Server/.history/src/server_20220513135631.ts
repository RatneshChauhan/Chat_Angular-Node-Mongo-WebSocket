...
io.on('connection', (socket) => {
  ...
  socket.on('joinUser', (user) => {
    ...
    socket.broadcast.emit('ONLINE', { text: 'online', userId: user.userID, email: user.userEmail })
  });

  socket.on('forceDisconnect', (user) => {
    console.log('Socket force disconnect !!! ', user)
    socket.broadcast.emit('OFFLINE', { text: 'offline', userId: user.userID, email: user.userEmail })
    console.log('Byeee bro! OFFLINE ')
    // socket.disconnect()
  })
