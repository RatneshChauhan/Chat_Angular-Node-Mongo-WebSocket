...
io.on('connection', (socket) => {
  ...
  socket.on('joinUser', (user) => {
    ...
    socket.broadcast.emit('ONLINE', { text: 'online', userId: user.userID, email: user.userEmail, userName: user.name })
    user.status = "ONLINE"
  ...
  socket.on('forceDisconnect', (user) => {
    socket.broadcast.emit('OFFLINE', { text: 'offline', userId: user.userID, email: user.userEmail, userName: user.name })
    user.status = "OFFLINE"
    Container.get<UserService>(UserService).updateUser(user)
      .then((data) => { console.log('status updated: ', data) })
